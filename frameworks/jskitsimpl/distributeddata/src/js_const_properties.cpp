...
static napi_status SetNamedProperty(napi_env env, napi_value& obj, const std::string& name, int32_t value)
{
    ...
    if (status != napi_ok) {
        ZLOGE("int32_t to napi_value failed!");
        return status;
    }
    status = napi_set_named_property(env, obj, name.c_str(), property);
    if (status != napi_ok) {
        ZLOGE("napi_set_named_property  failed!");
        return status;
    }
    return status;
}
...
static napi_value ExportSubscribeType(napi_env env)
{
    ...
    SetNamedProperty(env, subscribeType, "SUBSCRIBE_TYPE_LOCAL", (int32_t)JSUtil::SUBSCRIBE_LOCAL);
    SetNamedProperty(env, subscribeType, "SUBSCRIBE_TYPE_REMOTE", (int32_t)JSUtil::SUBSCRIBE_REMOTE);
    SetNamedProperty(env, subscribeType, "SUBSCRIBE_TYPE_ALL", (int32_t)JSUtil::SUBSCRIBE_LOCAL_REMOTE);
