...
int32_t FeatureStubImpl::OnAppUpdate(const std::string &bundleName, int32_t user, int32_t index, uint32_t tokenId)
{
    if (featureImpl_ == nullptr) {
        return -1;
    }
    return featureImpl_->OnAppUpdate(bundleName, user, index, tokenId);
}
