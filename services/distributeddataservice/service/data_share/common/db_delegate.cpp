...
std::shared_ptr<DBDelegate> DBDelegate::Create(DistributedData::StoreMetaData &metaData,
    const std::string &extUri, int32_t backupDbRule)
{
    if (metaData.tokenId == 0) {
        return std::make_shared<RdbDelegate>(metaData, NO_CHANGE_VERSION, true, extUri, backupDbRule);
    }
    std::shared_ptr<DBDelegate> store;
    stores_.Compute(metaData.tokenId,
        [&metaData, &store, extUri, backupDbRule](auto &,
            std::map<std::string, std::shared_ptr<Entity>> &stores) -> bool {
            ...
            store = std::make_shared<RdbDelegate>(metaData, NO_CHANGE_VERSION, true, extUri, backupDbRule);
            if (store->IsInvalid()) {
                store = nullptr;
