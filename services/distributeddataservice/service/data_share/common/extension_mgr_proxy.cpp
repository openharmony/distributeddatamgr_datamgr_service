...
{
    static std::shared_ptr<ExtensionMgrProxy> proxy(new ExtensionMgrProxy());
    return proxy;
}

bool ExtensionMgrProxy::Connect(
...
bool ExtensionMgrProxy::DisConnect(sptr<IRemoteObject> connect)
{
    std::lock_guard<std::mutex> lock(mutex_);
    if (ConnectSA()) {
