...
HWTEST_F(DocumentDBDataTest, UpsertDataTest001, TestSize.Level0)
{
    ...
    EXPECT_EQ(GRD_UpSertDoc(g_db, g_coll, "1234", document.c_str(), GRD_DOC_REPLACE), GRD_OK);
}
...
HWTEST_F(DocumentDBDataTest, UpsertDataTest003, TestSize.Level0)
{
    ...
    for (auto name : invalidName) {
        GLOGD("UpsertDataTest003: upsert data with collectionname: %s", name);
        EXPECT_EQ(GRD_UpSertDoc(g_db, name, "1234", document.c_str(), GRD_DOC_REPLACE), GRD_INVALID_ARGS);
    }
}
...
HWTEST_F(DocumentDBDataTest, UpsertDataTest005, TestSize.Level0)
{
    ...
    std::vector<const char *> invalidDocument = {
        nullptr,
        "",
        "GRD_123",
        "grd_123",
...
HWTEST_F(DocumentDBDataTest, UpsertDataTest007, TestSize.Level0)
{
    ...
    EXPECT_EQ(GRD_UpSertDoc(g_db, "collection_not_exists", "1234", val.c_str(), GRD_DOC_REPLACE), GRD_NO_DATA);
}
