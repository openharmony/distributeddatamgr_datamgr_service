...
端云服务负责本地数据与云端数据的双向同步，支持云开关管理、数据同步、云订阅、云数据共享、数据冲突处理和策略配置。

| 组件 | 说明 |
|------|------|
| **CloudServiceImpl** | 端云服务主入口，处理端云同步请求 |
| **SyncManager** | 端云同步管理器，管理同步任务生命周期 |
| **CloudServer/CloudDB/AssetLoader** | 云服务器、云数据库和资产加载的抽象接口 |
| **GeneralStore/AutoCache** | 通用存储与缓存管理（端云同步执行与缓存管理） |
...
services/distributeddataservice/
  service/cloud/                     # 端云服务实现
    cloud_service_impl.h/.cpp        # CloudServiceImpl
    cloud_service_stub.h/.cpp        # CloudServiceStub IPC Stub
    sync_manager.h/.cpp              # SyncManager
    sync_config.h/.cpp               # SyncConfig 同步配置管理
    cloud_notifier_proxy.h/.cpp      # CloudNotifierProxy 通知代理
    cloud_types_util.h/.cpp          # CloudTypesUtil 类型序列化工具
    cloud_value_util.h/.cpp          # CloudValueUtil 值转换工具
    cloud_data_translate.h/.cpp      # CloudDataTranslate 数据转换
    sync_strategies/                 # 同步策略实现
      network_sync_strategy.h/.cpp   # NetworkSyncStrategy（继承SyncStrategy）
  framework/include/cloud/           # 端云框架接口
    cloud_server.h / cloud_db.h / cloud_info.h / schema_meta.h
    cloud_event.h / sync_event.h / sync_strategy.h
    subscription.h / sharing_center.h / cloud_conflict_handler.h
    cloud_last_sync_info.h / asset_loader.h / cloud_extra_data.h
    cloud_config_manager.h / cloud_db_sync_config.h
    change_event.h / cloud_lock_event.h / cloud_mark.h / cloud_report.h
    cloud_share_event.h / cloud_sync_finished_event.h / make_query_event.h
  framework/include/store/           # 存储框架（端云依赖）
    general_store.h / auto_cache.h / general_value.h / store_info.h
  framework/include/metadata/        # 元数据（端云依赖）
    store_meta_data.h
```

> **外部依赖**：`cloud_types.h`（定义 Role/Confirmation/Privilege/Participant/StatisticInfo/Strategy 等）来自 `distributeddatamgr_relational_store`，由 `relational_store:cloud_data_inner` 构建；`cloud_service.h` 同源；`cloud_notifier.h` 同源。

| 模块 | 相对路径 | 功能 |
|------|----------|------|
| **CloudServiceImpl** | `services/distributeddataservice/service/cloud/cloud_service_impl.h` | 端云服务主入口 |
| **SyncManager** | `services/distributeddataservice/service/cloud/sync_manager.h` | 端云同步管理器 |
| **SyncStrategy** | `services/distributeddataservice/framework/include/cloud/sync_strategy.h` | 端云同步策略基类 |
| **NetworkSyncStrategy** | `services/distributeddataservice/service/cloud/sync_strategies/network_sync_strategy.h` | 网络同步策略（继承SyncStrategy，含Strategy枚举和StrategyInfo） |
| **SyncConfig** | `services/distributeddataservice/service/cloud/sync_config.h` | 同步配置管理 |
| **CloudDbSyncConfig** | `services/distributeddataservice/framework/include/cloud/cloud_db_sync_config.h` | 云数据库同步配置 |
| **CloudServer** | `services/distributeddataservice/framework/include/cloud/cloud_server.h` | 云服务器抽象接口 |
| **CloudDB** | `services/distributeddataservice/framework/include/cloud/cloud_db.h` | 云数据库抽象接口 |
| **CloudInfo** | `services/distributeddataservice/framework/include/cloud/cloud_info.h` | 云信息管理 |
| **SchemaMeta** | `services/distributeddataservice/framework/include/cloud/schema_meta.h` | Schema元数据 |
| **Subscription** | `services/distributeddataservice/framework/include/cloud/subscription.h` | 云订阅管理 |
| **SharingCenter** | `services/distributeddataservice/framework/include/cloud/sharing_center.h` | 云数据共享中心 |
| **CloudConflictHandler** | `services/distributeddataservice/framework/include/cloud/cloud_conflict_handler.h` | 云冲突处理 |
| **CloudNotifierProxy** | `services/distributeddataservice/service/cloud/cloud_notifier_proxy.h` | 端云通知代理（OnComplete/OnCloudSyncTrigger/OnSyncInfoNotify回调） |
| **CloudValueUtil** | `services/distributeddataservice/service/cloud/cloud_value_util.h` | 端云值转换（SharingUtil: Privilege/Participant/Confirmation双向转换; QueryResults/Status单向转换） |
| **CloudDataTranslate** | `services/distributeddataservice/service/cloud/cloud_data_translate.h` | 端云数据转换（RdbCloudDataTranslate: Asset↔Blob序列化） |
