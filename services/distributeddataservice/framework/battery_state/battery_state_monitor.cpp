...
#define LOG_TAG "BatteryStateMonitor"
...
public:
    int32_t Subscribe(const std::string &name, Observer observer) override
    {
        ...
        ZLOGI("battery observer subscribed, name:%{public}s, battery:%{public}d",
            name.c_str(), snapshot.batteryLevel);
        observer(snapshot);
        return 0;
    }

    int32_t Unsubscribe(const std::string &name) override
    {
        std::lock_guard<std::mutex> lock(mutex_);
        int32_t status = observers_.erase(name) == 0 ? -1 : 0;
    ...
    int32_t Start() override
    {
        ...
        ZLOGI("battery state monitor started, result:%{public}d, battery:%{public}d",
            started_, snapshot_.batteryLevel);
        return started_ ? 0 : -1;
    }
...
BatteryStateMonitor *BatteryStateMonitor::GetInstance()
{
