...
DistributedDB::DBStatus DistributedDBNbTestTools::GetKeys(DistributedDB::KvStoreNbDelegate &kvStoreNbDelegate,
    const DistributedDB::Key &keyPrefix, std::vector<DistributedDB::Key> &keys)
{
    return kvStoreNbDelegate.GetKeys(keyPrefix, keys);
}
